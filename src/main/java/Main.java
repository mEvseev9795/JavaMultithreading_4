...
public class Main {
    public static BlockingQueue<String> queueA = new ArrayBlockingQueue<>(100); //создаем три потокобезопасные очереди
    ...
    public static void main(String[] args) throws InterruptedException {
        Thread textGenerator = new Thread(() ->                          // создаем поток, который генерирует тексты
        {
        ...
        a.join(); //join(). Этот метод приостановит выполнение текущего потока до тех пор,
        b.join(); //пока другой поток не закончит свое выполнение
        c.join();
    }
